...
async def join_tournament(
    ...
    if not await db_actions.join_tournament(
        ...
        raise HTTPException(status_code=status.HTTP_406_NOT_ACCEPTABLE,detail="Неможливо приєжнатись до даного змагання")
    

@tournaments_router.patch("/{tournament_id}",status_code=status.HTTP_200_OK)
...
@tournaments_router.patch("/",status_code=status.HTTP_202_ACCEPTED)
async def add_vote(
...
async def get_results(
    ...
    results: List[Result] = await db_actions.get_results(tournament_id=tournament_id,db=db)
    results_model = []
...
@tournaments_router.get("/",status_code=status.HTTP_202_ACCEPTED,response_model=List[TournamentModelResponse])
